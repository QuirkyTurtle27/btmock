 ...
 * CreditCardSeeder
 *
 * This startup bean ensures that all predefined test credit cards used to
 * simulate BT API error scenarios are present in the MongoDB "creditCards"
 * collection.
 *
 * On application startup: - Iterates through the predefined list of test cards
 * (22 PANs mapped to action codes). - For each card, checks if it already
 * exists in the collection (by cardNumber). - If not found, persists the card
 * into MongoDB. - Logs which cards were inserted, or confirms that all were
 * already present.
 *
 * These seeded cards are used by the mock payment flow to trigger predictable
 * outcomes (success or specific decline codes) when testing
...
public class CreditCardSeeder {
	...
	private static final List<CreditCardEntity> SEED_CARDS = List.of(
			new CreditCardEntity("4444444444444444", "12/26", "444", "Card 444 - success"),
